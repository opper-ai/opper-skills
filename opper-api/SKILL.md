...
**None of these is the default answer — pick per use case.** Pin for determinism (evals, a guaranteed jurisdiction or price, a provider-specific behaviour); pool for redundancy without thinking about it; route for your own fallback chain, or when the decision depends on the request. Don't propose a new alias; point new chains at a route. A per-request chain exists on **chat completions only**: OpenRouter's `models` array (primary in `model`, the rest in `models`, tried in order; omit `model` and the first entry is the primary). No other compat surface reads it, and none reads `fallback_models` or a `model` array.
